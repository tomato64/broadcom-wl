...
sta_remove(eapd_wksp_t *nwksp, eapd_sta_t *sta)
{
	eapd_sta_t *sta_list;
	uint hash;
	/* WL-SHADOW-PATCH: eabuf was never declared here, because upstream ships
	 * EAPD_INFO defined to nothing, so the call below was never compiled.
	 * Restoring the macro (see eapd.h) exposed it. */
