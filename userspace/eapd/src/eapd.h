...
/* WL-SHADOW-PATCH: these two arrived defined to nothing - not behind BCMDBG,
 * not behind eapd_msg_level, just empty, in every Broadcom tree on this
 * machine (eapd_arm, eapd_arm_7, eapd_arm_714, EasyTomato). So eapd could not
 * report why it quit: "Unable to auto config. Quitting..." and "Command line
 * parsing error. Quitting..." both printed nothing and the process simply
 * vanished, which is exactly the diagnosis-free failure this project has
 * already lost cycles to.
 *
 * Restored to what eapd_msg_level plainly exists to gate. Still off by
 * default in the sense that matters: eapd_msg_level starts at EAPD_ERROR_VAL
 * (errors only, and errors here are fatal), and EAPD_INFO needs
 * "nvram set eapd_dbg=3".
 */
#define EAPD_ERROR(fmt, arg...)	do { \
		if (eapd_msg_level & EAPD_ERROR_VAL) \
			printf("EAPD_ERROR %s(%d): "fmt, __FUNCTION__, __LINE__ , ##arg); \
	} while (0)
#define EAPD_INFO(fmt, arg...)	do { \
		if (eapd_msg_level & EAPD_INFO_VAL) \
			printf("EAPD_INFO %s(%d): "fmt, __FUNCTION__, __LINE__ , ##arg); \
	} while (0)
